    ...
    async function setupContainer() {
      ...
      try {
        ...
        setLoadingState((prev) => ({
          ...prev,
          mounting: false,
          ready: true, // Mark as ready for manual input
        }));
        setCurrentStep(4);
        ...
        if (terminalRef.current?.writeToTerminal) {
          ...
          terminalRef.current.writeToTerminal("✅ Files mounted successfully!\r\n");
          terminalRef.current.writeToTerminal("👉 Please run the following commands manually:\r\n");
          terminalRef.current.writeToTerminal("   1. npm install\r\n");
          terminalRef.current.writeToTerminal("   2. npm run start\r\n");
          terminalRef.current.writeToTerminal("\r\n");
        }

        // Listen for server ready event (will fire when user manually starts server)
        instance.on("server-ready", (port: number, url: string) => {
